...
Этот файл — **единственный источник правды** для всех AI-агентов, работающих с
репозиторием. Инструменто-специфичные файлы (`CLAUDE.md` и т. п.) импортируют его и
добавляют только то, что осмысленно для конкретного инструмента.
...
Прогресс изменения — чекбоксы в его `tasks.md`. Шаги цикла (предложить → реализовать →
синхронизировать спеки → архивировать) описаны скиллами `openspec-propose`,
`openspec-apply-change`, `openspec-sync-specs`, `openspec-archive-change` в
`.agents/skills/`; под ними — `openspec` CLI.
...
- `AGENTS.md` (этот файл) — источник правды, tool-agnostic. Правила добавляются сюда.
- `CLAUDE.md` — тонкая обёртка: импорт `@AGENTS.md` плюс специфика Claude Code.
- `.agents/skills/<name>/SKILL.md` — реальные скиллы. Инструменты подключают их
  симлинками (Claude Code — `.claude/skills/<name>` → `../../.agents/skills/<name>`),
  так что новый скилл создаётся в `.agents/skills/`, а не в директории инструмента.
- Конфиги инструментов (`.claude/settings.json` и аналоги) не симлинкуются друг на
  друга — у них разные форматы и схемы; общее содержимое дублируется осознанно.
- `plugins/shortlinks/` — **публикуемый** плагин с навыком `shortlinks` (манифест
  `.claude-plugin/marketplace.json`), а не локальная конфигурация; в `.agents/` не входит.
